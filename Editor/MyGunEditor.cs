...
namespace GAME_MAIN.CodeBase.Editor
{
    ...
    [CustomEditor (typeof(MyGun))]
    public class MyGunEditor:UnityEditor.Editor
    {
        ...
        void UI_Impact()
        {
            ...
            EditorGUI.BeginChangeCheck();
            gun.type = (MyGun.ImpactType)EditorGUILayout.EnumPopup (gui_imp_tp, gun.type);
            if (EditorGUI.EndChangeCheck() == true)
            {
                foreach (MyGun scr in targets)
                {
        ...
        static void DrawGizmosSelected (MyGun gun, GizmoType gizmoType)
        {
            // Ray
            if (gun.showRay == true)
            {
