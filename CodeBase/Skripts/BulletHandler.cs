                ...
                if (i < MaxBulletUpdatesPerFrame)
                {
                    ...
                    while (processAgain)
                    {
                        ...
                        if (Physics.Raycast(ray, out hit, dirMag, cWeapon.HitMask))
                        {
                            ...
                            if (hitTrans.TryGetComponent(out EnemyBodyPart part))
                            {
                                bodyPart = part;
                            }
                            
                            if (hitTrans.TryGetComponent(out DestroyTarget target))
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    destroyTarget.ImpactPoint = cBullet.LastPos;
                                    destroyTarget.ShootPosition =hit.point ;
                                    destroyTarget.Strength = BulletEnergy;
