...
using System.Linq;
using RayFire.Scripts.Classes;
using RayFire.Scripts.Classes.Rigid;
...
namespace GAME_MAIN.CodeBase.Logic
{
    public class MyGun : MonoBehaviour
    {
        ...
        public void Shoot (Vector3 shootPos, Vector3 shootVector)
        {
            ...
            DestroyTarget destroyTarget = null;
            /*if (hit.collider.TryGetComponent(out EnemyBodyPart enemyBodyPart))
            {
            ...
            if (desTarget == true)
            {
                destroyTarget = hit.collider?.GetComponent<DestroyTarget>();
                if (destroyTarget != null)
                {
                    destroyTarget.ImpactPoint = impactPoint;
                    destroyTarget.ShootPosition = shootPos;
                    destroyTarget.ImpactNormal = impactNormal;
                    destroyTarget.Strength = strength;
                    destroyTarget.Demolish();
                }
            }
        ...
        void AddForce(List<Rigidbody> impactRbList, Vector3 impactPoint, Vector3 shootVector)
        {
            ...
            // Apply force
            List<Rigidbody> newRigd;
            List<GameObject> newUnRigd;
